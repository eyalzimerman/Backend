...
router.get("/", (request, response) => {
  try {
    ...
    for (let i = 0; i < allUsers.length; i++) {
      let body = fs.readFileSync(`./backend/bins/${allUsers[i]}`, {
        encoding: "utf8",
        flag: "r",
      });
...
router.get("/:id", checkID, checkBin, (request, response) => {
  try {
    ...
    let body = fs.readFileSync(`./backend/bins/${id}.json`, {
      encoding: "utf8",
      flag: "r",
    });
    body = JSON.parse(body);
...
router.put("/:id", checkID, checkBin, blankBinCheck, (request, response) => {
  try {
    let allUsers = fs.readdirSync("./backend/bins");
    const { id } = request.params;
    const { body } = request;
  ...
  try {
    ...
    for (let i = 0; i < allUsers.length; i++) {
      if (allUsers[i] === `${id}.json`) {
        ...
        response.status(200).send({ message: "task delete successfully" });
      }
    }
  } catch (e) {
    response
